...
class KendoModel
{
    ...
    private function sort($propertyNames, $sort) {
        ...
        if ($count > 0) {
            ...
            for ($index = 0; $index < $count; $index ++) {
                ...
                //              print_r($properties);
                //////////////////////////////
                foreach ($propertyNames as $key => $column) {
                    if (preg_match("/\bas\b/i", $column)) {
                        //echo " -- ".$column;
                        $pos = stripos($column, ' as ');
                        //echo " -- ".
                        $propertyNames[] = trim(substr($column, $pos + 4));
                    }
                }
                //print_r($properties);
                /////////////////////////////
    ...
    private function where($properties, $filter, $all) {
        ...
        //////////////////////////////
        foreach ($propertyNames as $key => $column) {
            if (preg_match("/\b$field\b/i", $column)) {
                $pos = stripos($column, ' as ');
                $map_field = trim(substr($column, 0, $pos));
                if (!empty($map_field)) {
                    $propertyNames[] = $field = $map_field;
                }
            }
        }
    ...
    public function read($table, $properties, $request = null) {
        ...
        //@dev: Open this line to debug SQL queries passed to KendoUI
        //dd($sql);
